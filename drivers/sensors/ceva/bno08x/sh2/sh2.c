...
static void sensorhubControlHdlr(
        ...
        if (reportLen == 0) {
            ...
            if (reportId == SENSORHUB_COMMAND_RESP) {
                ...
                if ((pResp->command ==
                    ...
                    // Ignore this.  EXECUTABLE_DEVICE_RESP_RESET_COMPLETE makes
                    // it redundant.
                }
                ...
                if (pSh2->eventCallback) {
                    ...
                    sh2AsyncEvent.sh2SensorConfigResp.sensorConfig
                        ...
                        ((pGetFeatureResp->flags &
                          FEAT_CHANGE_SENSITIVITY_ENABLED) != 0);
                    sh2AsyncEvent.sh2SensorConfigResp.sensorConfig
                        .changeSensitivityRelative =
                        ((pGetFeatureResp->flags &
                          FEAT_CHANGE_SENSITIVITY_RELATIVE) != 0);
                    sh2AsyncEvent.sh2SensorConfigResp.sensorConfig
                        .wakeupEnabled =
                        ((pGetFeatureResp->flags & FEAT_WAKE_ENABLED) != 0);
                    sh2AsyncEvent.sh2SensorConfigResp.sensorConfig
                        .alwaysOnEnabled =
                        ((pGetFeatureResp->flags & FEAT_ALWAYS_ON_ENABLED) !=
                         0);
                    sh2AsyncEvent.sh2SensorConfigResp.sensorConfig
                        .changeSensitivity = pGetFeatureResp->changeSensitivity;
                    sh2AsyncEvent.sh2SensorConfigResp.sensorConfig
                        .reportInterval_us = pGetFeatureResp->reportInterval_uS;
                    sh2AsyncEvent.sh2SensorConfigResp.sensorConfig
                        .batchInterval_us = pGetFeatureResp->batchInterval_uS;
                    sh2AsyncEvent.sh2SensorConfigResp.sensorConfig
                        .sensorSpecific = pGetFeatureResp->sensorSpecific;
...
static int opProcess(sh2_t* pSh2, const sh2_Op_t* pOp) {
    ...
    while ((pSh2->pOp != 0) && ((pOp->timeout_us == 0) ||
...
static uint64_t touSTimestamp(
    uint32_t hostInt, int32_t referenceDelta, uint16_t delay) {
...
static void sensorhubInputHdlr(
    sh2_t* pSh2, uint8_t* payload, uint16_t len, uint32_t timestamp) {
    ...
    while (cursor < len) {
        ...
        if (reportLen == 0) {
            ...
            if (reportId == SENSORHUB_BASE_TIMESTAMP_REF) {
                ...
                event.timestamp_uS =
                    touSTimestamp(timestamp, referenceDelta, delay);
...
static void sensorhubInputNormalHdlr(
    void* cookie, uint8_t* payload, uint16_t len, uint32_t timestamp) {
    sh2_t* pSh2 = (sh2_t*)cookie;
...
static void sensorhubInputWakeHdlr(
    void* cookie, uint8_t* payload, uint16_t len, uint32_t timestamp) {
    sh2_t* pSh2 = (sh2_t*)cookie;
...
static void sensorhubInputGyroRvHdlr(
    void* cookie, uint8_t* payload, uint16_t len, uint32_t timestamp) {
    sh2_t* pSh2 = (sh2_t*)cookie;
    sh2_SensorEvent_t event;
    uint16_t cursor = 0;
...
static void executableDeviceHdlr(
    void* cookie, uint8_t* payload, uint16_t len, uint32_t timestamp) {
    (void)timestamp;  // unused
...
static void getProdIdRx(sh2_t* pSh2, const uint8_t* payload, uint16_t len) {
    ...
    if (resp->reportId != SENSORHUB_PROD_ID_RESP)
        return;
...
static void getSensorConfigRx(
    sh2_t* pSh2, const uint8_t* payload, uint16_t len) {
    (void)len;  // unused
    ...
    if (resp->reportId != SENSORHUB_GET_FEATURE_RESP)
        return;
    if (resp->featureReportId != pSh2->opData.getSensorConfig.sensorId)
        return;
...
static int setSensorConfigStart(sh2_t* pSh2) {
    ...
    if (pConfig->wakeupEnabled)
        flags |= FEAT_WAKE_ENABLED;
    if (pConfig->alwaysOnEnabled)
        flags |= FEAT_ALWAYS_ON_ENABLED;
    if (pConfig->sniffEnabled)
        flags |= FEAT_SNIFF_ENABLED;
...
static void getFrsRx(sh2_t* pSh2, const uint8_t* payload, uint16_t len) {
    ...
    if (resp->reportId != SENSORHUB_FRS_READ_RESP)
        return;
...
static void stuffMetadata(sh2_SensorMetadata_t* pData, uint32_t* frsData) {
    ...
    if (pData->revision == 0) {
        ...
        memcpy(
            pData->sensorSpecific, (uint8_t*)&frsData[9],
        ...
        memcpy(
            pData->sensorSpecific, (uint8_t*)&frsData[10],
            pData->sensorSpecificLen);
    ...
    // Copy vendor id
    memcpy(
        pData->vendorId, (uint8_t*)&frsData[vendorIdOffset],
        pData->vendorIdLen);
}
...
static bool wrongResponse(sh2_t* pSh2, CommandResp_t* resp) {
    if (resp->reportId != SENSORHUB_COMMAND_RESP)
        return true;
    if (resp->command != pSh2->lastCmdId)
        return true;
    if (resp->commandSeq != pSh2->cmdSeq)
        return true;
...
static int getCountsStart(sh2_t* pSh2) {
    return sendCmd2(
        pSh2, SH2_CMD_COUNTS, SH2_COUNTS_GET_COUNTS,
...
static int sendCmdStart(sh2_t* pSh2) {
    int status = sendCmd(
        pSh2, pSh2->opData.sendCmd.req.command, pSh2->opData.sendCmd.req.p);
...
static int setCalConfigStart(sh2_t* pSh2) {
    ...
    p[6] =
        (pSh2->opData.calConfig.sensors & SH2_CAL_ZERO_GYRO_CONTROL_MASK) >> 5;
...
static void forceFlushRx(sh2_t* pSh2, const uint8_t* payload, uint16_t len) {
    ...
    // Ignore message if it doesn't pertain to this operation
    if (resp->reportId != SENSORHUB_FLUSH_COMPLETED)
        return;
    if (resp->sensorId != pSh2->opData.forceFlush.sensorId)
        return;
 ...
 */
int sh2_open(
    sh2_Hal_t* pHal, sh2_EventCallback_t* eventCallback, void* eventCookie) {
    sh2_t* pSh2 = &_sh2;
    ...
    shtp_listenChan(
        pSh2->pShtp, CHAN_SENSORHUB_CONTROL, sensorhubControlHdlr, pSh2);
    shtp_listenChan(
        pSh2->pShtp, CHAN_SENSORHUB_INPUT, sensorhubInputNormalHdlr, pSh2);
    shtp_listenChan(
        pSh2->pShtp, CHAN_SENSORHUB_INPUT_WAKE, sensorhubInputWakeHdlr, pSh2);
    shtp_listenChan(
        pSh2->pShtp, CHAN_SENSORHUB_INPUT_GIRV, sensorhubInputGyroRvHdlr, pSh2);

    // Register EXECUTABLE handlers
    shtp_listenChan(
        pSh2->pShtp, CHAN_EXECUTABLE_DEVICE, executableDeviceHdlr, pSh2);
