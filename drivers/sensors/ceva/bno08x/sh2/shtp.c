...
static int txProcess(
    shtp_t* pShtp, uint8_t chan, const uint8_t* pData, uint32_t len) {
    int status = SH2_OK;
    ...
    while (remaining > 0) {
        ...
        transferLen =
            min_u16(remaining, SH2_HAL_MAX_TRANSFER_OUT - SHTP_HDR_LEN);
        ...
        while (status == 0) {
            shtp_service(pShtp);
            status =
                pShtp->pHal->write(pShtp->pHal, pShtp->outTransfer, lenField);
        }
...
static void rxAssemble(
    shtp_t* pShtp, uint8_t* in, uint16_t len, uint32_t t_us) {
    uint16_t payloadLen;
    bool continuation;
    uint8_t chan = 0;
    uint8_t seq = 0;
    ...
    if (seq != pShtp->chan[chan].nextInSeq) {
        if (pShtp->eventCallback) {
            pShtp->eventCallback(pShtp->eventCookie, SHTP_BAD_SN);
        }
    }
    ...
    // Discard earlier assembly in progress if the received data doesn't match
    // it.
    if (pShtp->inRemaining) {
        // Check this against previously received data.
        if (!continuation || (chan != pShtp->inChan) ||
            (seq != pShtp->chan[chan].nextInSeq) ||
            (payloadLen - SHTP_HDR_LEN != pShtp->inRemaining)) {
            if (pShtp->eventCallback) {
                pShtp->eventCallback(pShtp->eventCookie, SHTP_BAD_FRAGMENT);
            }
            ...
            if (pShtp->eventCallback) {
                pShtp->eventCallback(
                    pShtp->eventCookie, SHTP_INTERRUPTED_PAYLOAD);
            }
        }
    }
    ...
    memcpy(
        pShtp->inPayload + pShtp->inCursor, in + SHTP_HDR_LEN,
        len - SHTP_HDR_LEN);
    ...
    if (pShtp->inRemaining == 0) {
        ...
        if (pShtp->chan[chan].callback != 0) {
            pShtp->chan[chan].callback(
                pShtp->chan[chan].cookie, pShtp->inPayload, pShtp->inCursor,
                pShtp->inTimestamp);
        }
    }
}
...
// Register the pointer of the callback function for reporting asynchronous
// events
...
int shtp_listenChan(
    void* pInstance, uint8_t channel, shtp_Callback_t* callback, void* cookie) {
    shtp_t* pShtp = (shtp_t*)pInstance;
...
int shtp_send(
    void* pInstance, uint8_t channel, const uint8_t* payload, uint16_t len) {
    shtp_t* pShtp = (shtp_t*)pInstance;
