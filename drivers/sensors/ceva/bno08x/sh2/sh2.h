...
typedef struct sh2_SensorConfig {
        ...
        bool changeSensitivityRelative; /**< @brief Change reports relative (vs
                                           absolute) */
        ...
        uint32_t sensorSpecific; /**< @brief See SH-2 Reference Manual for
                                    details. */
} sh2_SensorConfig_t;
...
typedef struct sh2_ErrorRecord {
        ...
        uint8_t source;   /**< @brief 1-MotionEngine, 2-MotionHub, 3-SensorHub,
                             4-Chip  */
...
typedef enum sh2_TareBasis {
    ...
    SH2_TARE_BASIS_GAMING_ROTATION_VECTOR =
        1, /**< @brief Use Game Rotation Vector */
    SH2_TARE_BASIS_GEOMAGNETIC_ROTATION_VECTOR =
        2, /**< @brief Use Geomagnetic R.V. */
} sh2_TareBasis_t;
...
typedef enum sh2_TareAxis {
    ...
    SH2_TARE_CONTROL_VECTOR_X =
        (1 << 3), /**< @brief Use X axis of source and frame to perform tare */
    SH2_TARE_CONTROL_VECTOR_Y =
        (0 << 3), /**< @brief Use Y axis of source and frame to perform tare */
    SH2_TARE_CONTROL_VECTOR_Z =
        (2 << 3), /**< @brief Use Z axis of source and frame to perform tare */
    SH2_TARE_CONTROL_SEQUENCE_DEFAULT =
        (0 << 5), /**< @brief Tare "typical" toration for source/axis
                     combination */
    SH2_TARE_CONTROL_SEQUENCE_PRE =
        (1 << 5), /**< @brief Apply to pre-rotation (tare world to device) */
    SH2_TARE_CONTROL_SEQUENCE_POST =
        (2 << 5), /**< @brief Apply to post-rotation (tare device to world) */
} sh2_TareAxis_t;
 ...
 *
 * Represents reset events and other non-sensor events received from SH-2 sensor
 * hub.
 */
