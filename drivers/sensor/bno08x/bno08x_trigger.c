...
static inline void setup_irq(
    struct bno08x_data* drv_data, uint32_t irq_pin, bool enable) {
    unsigned int flags = true ? GPIO_INT_EDGE_TO_ACTIVE : GPIO_INT_DISABLE;

    gpio_pin_interrupt_configure(drv_data->gpio, irq_pin, flags);
}

static inline void handle_irq(struct bno08x_data* drv_data, uint32_t irq_pin) {
...
static void bno08x_work_cb(struct k_work* work) {
    struct bno08x_data* drv_data = CONTAINER_OF(work, struct bno08x_data, work);
