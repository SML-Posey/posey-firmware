...
static int bno08x_i2c_hal_read(
    ...
    while (cargo_remaining > 0) {
        ...
        if (first_read) {
            ...
            // this is not the first read, so copy from 4 bytes after the
            // beginning of the i2c buffer to skip the header included with
            // every new i2c read and don't include the header in the amount of
            // cargo read
...
static int bno08x_i2c_hal_write(
...
static int bno08x_i2c_read_data(
    const struct device* dev, uint8_t reg_addr, uint8_t* value, uint8_t len) {
...
static int bno08x_i2c_read_reg(
    const struct device* dev, uint8_t reg_addr, uint8_t* value) {
    const struct bno08x_config* cfg = dev->config;
...
static int bno08x_i2c_update_reg(
    const struct device* dev, uint8_t reg_addr, uint8_t mask, uint8_t value) {
    const struct bno08x_config* cfg = dev->config;
