 ...
 * @date 10 Nov 2015
 * @brief Support for converting sensor events (messages) into natural data
 * structures.
 *
 */
...
typedef struct sh2_SensorValue {
         ...
         *   3 - Accuracy high
         */
        uint8_t status; /**< @brief bits 7-5: reserved, 4-2: exponent delay,
                           1-0: Accuracy */

        uint64_t timestamp; /**< [uS] */

        uint32_t
            delay; /**< @brief [uS] value is delay * 2^exponent (see status) */
