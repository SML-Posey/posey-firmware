...
void deep_sleep() {
    ...
    int sleep_m =
        (int)(timeout.ticks * 1.0 / CONFIG_SYS_CLOCK_TICKS_PER_SEC / 60.0);
    int sleep_s = (int)(timeout.ticks * 1.0 / CONFIG_SYS_CLOCK_TICKS_PER_SEC -
                        sleep_m * 60);

    LOG_INF("Sleeping for %d minutes %d seconds...", sleep_m, sleep_s);
    // static const struct pm_state_info suspend_state = {
    //     PM_STATE_SUSPEND_TO_IDLE, 0, 0};
...
float read_Vbatt(const bool verbose) {
    ...
    if (err) {
        LOG_ERR(
            "ADC sequence init failed with error %d - %s", err, strerror(err));
        return 0.0;
    }
    ...
    int32_t Vain_mv = buf;
    err = adc_raw_to_millivolts_dt(&adc_channels[0], &Vain_mv);
    if (err) {
        LOG_ERR(
            "ADC raw to millivolts failed with error %d - %s", err,
            strerror(err));
        return 0.0;
    }
