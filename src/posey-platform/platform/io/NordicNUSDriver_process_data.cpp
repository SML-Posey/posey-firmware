...
int write_flash(const uint8_t* const data, uint16_t size) {
    ...
    if (rc != 0)
        LOG_ERR("FAILED to write %d bytes at offset %ld", size, flash_offset);
    else
        LOG_DBG("Wrote %d bytes at offset %ld", size, flash_offset);
...
extern "C" uint16_t read_flash(
    const uint32_t offset, uint8_t* data, uint16_t size) {
    ...
    if (end_offset > data_size) {
        ...
        LOG_WRN(
            "Trying to read past offset %d to %d, decreasing read size to %d",
            data_size, end_offset, size);
    }
    int rc = flash_read(flash_dev, offset, data, size);
    if (rc != 0)
        LOG_ERR("FAILED to read %d bytes at offset %d", size, offset);
...
extern "C" void process_data(
    ...
    if ((++iter % 1000) == 0) {
        ...
        LOG_INF(
            "Recording in progress: %.2f of %.2f MB (%.2f%%)", used, flash_size,
            pct);
...
extern "C" bool erase_used_flash() {
    ...
    uint32_t sectors =
        static_cast<uint32_t>((1.0 * flash_offset) / FLASH_SECTOR_SIZE) + 1;
...
extern "C" bool init_flash() {
    ...
    LOG_INF(
        "Existing flash started %s, %d bytes.", device_config.data_dt,
